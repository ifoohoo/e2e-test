...
function advanceAuthor(req, projectRoot, pluginRoot) {
  ...
  if (stage === 'inspect') {
    ...
    result = repairWorker({ ...common, inputs: {
      prior, artifact: readIntermediate(runRoot, 'artifact.json'), findings: review.findings, repairPlan: req.repairPlan || {},
    } });
...
function outputSet(output) {
  ...
  return { artifact: output, matrix: `${base}.matrix${extension}`, manifest: `${base}.package${extension}` };
}
