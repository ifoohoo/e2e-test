...
import { loadCapabilityCatalog, projectCapabilitiesForHelp } from './lib/capability-catalog-loader.mjs';
...
const DIAGNOSTICS = {
  ...
  CAPABILITY_CATALOG_UNAVAILABLE: { code: 'CAPABILITY_CATALOG_UNAVAILABLE', severity: 'error' },
  CAPABILITY_CATALOG_INVALID: { code: 'CAPABILITY_CATALOG_INVALID', severity: 'error' },
};
...
if (request.service === 'help' || (request.service === 'default' && request.intent === 'help')) {
  ...
  // Load capability catalog — fail-close: never fall back to handwritten arrays
  const { catalog, digest: catalogDigest, errors: catalogErrors } = loadCapabilityCatalog(pluginRoot);
  if (!catalog) {
    emitFailure(new ServiceError('CAPABILITY_CATALOG_UNAVAILABLE', [DIAGNOSTICS.CAPABILITY_CATALOG_UNAVAILABLE]));
  }
  if (catalogErrors && catalogErrors.length > 0) {
    emitFailure(new ServiceError('CAPABILITY_CATALOG_INVALID', catalogErrors.map(detail => ({
      code: 'CAPABILITY_CATALOG_INVALID', severity: 'error', detail,
    }))));
  }
  const capabilities = projectCapabilitiesForHelp(catalog);
